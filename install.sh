...
REPO="${DOCKERMGRREPO:-https://github.com/dockermgr}/whoogle"
APPVERSION="$(__appversion "$REPO/raw/${GIT_REPO_BRANCH:-main}/version.txt")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Defaults variables
APPNAME="whoogle"
INSTDIR="$HOME/.local/share/dockermgr/whoogle"
APPDIR="$HOME/.local/share/srv/docker/whoogle"
DATADIR="$HOME/.local/share/srv/docker/whoogle/files"
...
if cmd_exists docker-compose && [ -f "$INSTDIR/docker-compose.yml" ]; then
  ...
  __sudo docker run -d \
    ...
    -e WHOOGLE_CONFIG_TOR="${WHOOGLE_CONFIG_TOR:-0}" \
